...
function updateBasketTable(basketItemId, res)
{
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED'))
			BX('allSum_FORMATED').innerHTML = '<strong>'+res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '').replace(/руб./g,'')+'</strong> <span>рублей</span>';
...
function recalcBasketAjax(params)
{
	...
	items.rows=items.getElementsByTagName('li');
	//console.log(items.rows);
	if (!!items && items.rows.length > 0)
	{
		for (i = 0; i < items.rows.length-1; i++)
